...
public class RankingListCharmTopProvider extends ItemViewProvider<List<CharmRankingListBean.DataBean>, RankingListCharmTopProvider.ViewHolder> {
    ...
    public RankingListCharmTopProvider(BaseFragment fragment) {
    ...
    @Override
    protected void onBindViewHolder(@NonNull final ViewHolder holder, @NonNull final List<CharmRankingListBean.DataBean> bean) {
    ...
    private void provcessHolder1(final CharmRankingListBean.DataBean bean, ViewHolder holder) {
        Glide.with(mContext)
                .load(bean.avatar)
        ...
        holder.mTvTopRangklingListCharm1.setText("魅力值：" + bean.total);

        AppUtils.setDataAgeAndGender(bean.age, bean.sex, holder.mTvTopRangklingListAge1, mDrawableBoySmall, mDrawableGirlSmall);

        AppUtils.initVisiableWithGone(holder.mTvTopRangklingListViplevel1, bean.level != 0);
    }
    private void provcessHolder2(final CharmRankingListBean.DataBean bean, ViewHolder holder) {
        Glide.with(mContext)
                .load(bean.avatar)
                .apply(bitmapTransform(new RoundedCornersTransformation(100, 0, RoundedCornersTransformation.CornerType.ALL)))
        ...
        holder.mTvTopRangklingListCharm2.setText("魅力值：" + bean.total);

        AppUtils.setDataAgeAndGender(bean.age, bean.sex, holder.mTvTopRangklingListAge2, mDrawableBoySmall, mDrawableGirlSmall);

        AppUtils.initVisiableWithGone(holder.mTvTopRangklingListViplevel2, bean.level != 0);
    }

    private void provcessHolder3(final CharmRankingListBean.DataBean bean, ViewHolder holder) {
        Glide.with(mContext)
                .load(bean.avatar)
                .apply(bitmapTransform(new RoundedCornersTransformation(100, 0, RoundedCornersTransformation.CornerType.ALL)))
        ...
        holder.mTvTopRangklingListCharm3.setText("魅力值：" + bean.total);


        AppUtils.setDataAgeAndGender(bean.age, bean.sex, holder.mTvTopRangklingListAge3, mDrawableBoySmall, mDrawableGirlSmall);
