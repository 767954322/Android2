...
public class listcityAdapter extends BaseAdapter{
    ...
    private List<addinfobean.DataBean.CityBean> list;
    private int selectedPosition = 0;// 选中的位置
    public void setSelectedPosition(int position) {
        selectedPosition = position;
    }
    public listcityAdapter(Context context, List<addinfobean.DataBean.CityBean> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.list_right, null);
            holder = new ViewHolder();

            holder.nameTV = (TextView) convertView.findViewById(R.id.list_view_item_text_right);
