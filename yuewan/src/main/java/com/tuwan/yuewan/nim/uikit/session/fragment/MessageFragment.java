package com.tuwan.yuewan.nim.uikit.session.fragment;
...
import com.tuwan.yuewan.nim.uikit.ait.AitManager;
...
import com.tuwan.yuewan.nim.uikit.session.actions.LocationAction;
import com.tuwan.yuewan.nim.uikit.session.actions.VideoAction;
...
import com.tuwan.yuewan.nim.uikit.session.module.input.InputPanel2;
import com.tuwan.yuewan.nim.uikit.session.module.list.MessageListPanelEx;
...
public class MessageFragment extends TFragment implements ModuleProxy {
    ...
    private MessageBtmView mMessageBtmView;

    @Override
    public void onActivityCreated(Bundle savedInstanceState) {
        super.onActivityCreated(savedInstanceState);
        mMessageBtmView = (MessageBtmView) rootView.findViewById(R.id.message_btn_view);
        parseIntent();

        if (TextUtils.equals(Constants.SYSTEM_NOTICE_ACCOUNT, sessionId) || TextUtils.equals(Constants.SYSTEM_ORDER_ACCOUNT, sessionId) || TextUtils.equals(Constants.SYSTEM_PUSH_ACCOUNT, sessionId) || TextUtils.equals(Constants.SYSTEM_PUSH_HONGBAO, sessionId)) {
            //官方公告的底部栏不可见
            mMessageBtmView.setVisibility(View.GONE);
        } else {
            mMessageBtmView.setVisibility(View.VISIBLE);
        }
    }
    ...
    private void parseIntent() {
        ...
        if (!TextUtils.equals(container.account, "官方公告")) {
            if (inputPanel == null) {
                inputPanel = new InputPanel2(container, mMessageBtmView, getActionList(), getActivity());
            } else {
    ...
    public void shouldCollapseInputPanel() {
        LogUtil.e("shouldCollapseInputPanel======================");
        if (inputPanel != null) {
            inputPanel.collapse();
        }
