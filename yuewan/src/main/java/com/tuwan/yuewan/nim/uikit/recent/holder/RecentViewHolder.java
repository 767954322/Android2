...
import com.tuwan.yuewan.nim.uikit.common.ui.drop.DropFake;
...
public abstract class RecentViewHolder extends RecyclerViewHolder<BaseQuickAdapter, BaseViewHolder, RecentContact> {
    ...
    private void updateBackground(BaseViewHolder holder, RecentContact recent, int position) {
        bottomLine.setVisibility(getAdapter().isLastDataItem(position) ? View.GONE : View.VISIBLE);
    ...
    protected void loadPortrait(RecentContact recent) {
        ...
        if (recent.getSessionType() == SessionTypeEnum.P2P) {
            if(TextUtils.equals(recent.getFromAccount(), Constants.SYSTEM_NOTICE_ACCOUNT)){
                imgHead.setImageResource(R.drawable.ic_avart_session_notice);
                return;
