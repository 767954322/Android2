...
import com.tuwan.yuewan.nim.demo.avchat.widgets.ToggleState;
import com.tuwan.yuewan.nim.demo.avchat.widgets.ToggleView;
...
//    private View faceUnityRoot;
    ...
    public void onClick(View v) {
        if(v.getId()==R.id.avchat_video_logout){
            listener.onHangUp();
        }else if(v.getId()==R.id.refuse){
            listener.onRefuse();
        }else if(v.getId()==R.id.receive){
            listener.onReceive();
        }else if(v.getId()==R.id.avchat_video_mute){
            listener.toggleMute();
        }else if(v.getId()==R.id.avchat_video_switch_audio){
            if (isInSwitch) {
                Toast.makeText(context, R.string.avchat_in_switch, Toast.LENGTH_SHORT).show();
            } else {
                listener.videoSwitchAudio();
            }
        }else if(v.getId()==R.id.avchat_switch_camera){
            listener.switchCamera();
        }else if(v.getId()==R.id.avchat_close_camera){
            listener.closeCamera();
        }else if(v.getId()==R.id.avchat_video_record){
