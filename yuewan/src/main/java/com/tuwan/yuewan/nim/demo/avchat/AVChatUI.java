...
import com.tuwan.yuewan.nim.demo.avchat.widgets.MultiSelectDialog;
...
public class AVChatUI implements AVChatUIListener {
    ...
    public boolean init(AVChatSurface.TouchZoneCallback cb) {
        ...
        avChatAudio = new AVChatAudio(context, root, this, this);
     ...
     * 拨打音视频
     */
    public void outGoingCalling(String account, final AVChatType callTypeEnum, int sid) {
        AVChatSoundPlayer.instance().play(AVChatSoundPlayer.RingerTypeEnum.CONNECTING);

        this.receiverId = account;

        AVChatManager.getInstance().muteLocalAudio(false);
        callCheck(sid, callTypeEnum);
    }

    /**
     * 先检查被叫方状态
     */
    private void callCheck(int sid, final AVChatType callTypeEnum) {
        avChatAudio.onCallStateChange(CallStateEnum.OUTGOING_AUDIO_CALLING);

        ServiceFactory.getNoCacheInstance()
                .createService(YService.class)
                .voiceCall("json", sid, 4, 0)
                ...
                .subscribe(new CommonObserver<CallBean>() {
                    ...
                    public void onNext(@NonNull CallBean result) {
                        ...
                        if (result.error == 0) {
                            ...
                            if (result.diamond < result.price) {
                                closeRtc();
                                closeSessions(22);
                                return;
                            }
                            ...
                            if (avChatAudio != null) {
                                avChatAudio.setUserInfo(result.sex, result.age);
                                avChatAudio.setDiamond(result.diamond);
                                avChatAudio.setPrice(result.price);
                            }
     ...
     * @param callTypeEnum
     */
    private void call(final AVChatType callTypeEnum) {
    ...
    public void showQuitToast(int code) {
        switch (code) {
            ...
            case AVChatExitCode.LOCAL_CALL_BUSY:
                ...
                break;
            case AVChatExitCode.DIAMOND_NO:
                Toast.makeText(context, "您的钻石不足，请先充值", Toast.LENGTH_SHORT).show();
                break;
            case AVChatExitCode.REJECT:
                Toast.makeText(context, "对方拒绝了您的通话请求", Toast.LENGTH_SHORT).show();
                break;
            case AVChatExitCode.PEER_NO_RESPONSE:
                Toast.makeText(context, "暂时无人接听，请稍候再拨", Toast.LENGTH_SHORT).show();
                break;
            default:
                break;
        }
    ...
    public void toggleMute() {
        if (!isCallEstablish.get()) { // 连接未建立，在这里记录静音状态
            ...
            if (!AVChatManager.getInstance().isLocalAudioMuted()) { // isMute是否处于静音状态
                ...
                avChatAudio.setMute(false);
            } else {
                // 打开音频
                AVChatManager.getInstance().muteLocalAudio(false);
                avChatAudio.setMute(true);
            }
        }
    }
    ...
    public void toggleSpeaker() {
        boolean open = AVChatManager.getInstance().speakerEnabled();
        AVChatManager.getInstance().setSpeaker(!open); // 设置扬声器是否开启
        if (avChatAudio != null) {
            avChatAudio.setHansfree(!open);
        }
    }
    ...
    public void showGiftDialog() {
        int accid = 0;
        if (receiverId != null) {
            accid = Integer.parseInt(receiverId);
        }

        aVChatListener.giftDialog(accid);
    }
    ...
    public void setGift(VoiceGiftAttachment voiceGiftAttachment) {
        avChatAudio.setGift(voiceGiftAttachment);
    }
    ...
    public void setDiamond(int diamond) {
        if (avChatAudio != null) {
            avChatAudio.setDiamond(diamond);
            avChatAudio.countDownTimer();
        }
    }
    ...
    public void callStart() {
        ...
        ServiceFactory.getNoCacheInstance()
                .createService(YService.class)
                .voiceStart("json", receiverId, avChatData.getChatId())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new CommonObserver<CallBean>() {
                    @Override
    ...
    public void callEnd() {
        ...
        ServiceFactory.getNoCacheInstance()
                .createService(YService.class)
                .voiceEnd("json", avChatData.getChatId())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new CommonObserver<CallBean>() {
                    @Override
     ...
     * 未接调用
     */
    public void callEndnocome(final String type) {
        if (avChatData == null) {
            return;
        }
        ServiceFactory.getNoCacheInstance()
                .createService(YService.class)
                .voiceEndnocome("json", mId, type)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new CommonObserver<CallBean>() {
                    @Override
    ...
    public void callUserInfo() {
        ServiceFactory.getNoCacheInstance()
                .createService(YService.class)
                .voiceCallUserInfo("json", receiverId)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new CommonObserver<CallUserInfoBean>() {
                    @Override
                    public void onNext(@NonNull CallUserInfoBean result) {
                        super.onNext(result);

                        if (result.error == 0) {
                            avChatAudio.setUserInfo(result.sex, result.age);
                            avChatAudio.showIncomeTips(result.price);
                            avChatAudio.setPrice(result.price);
                            avChatAudio.setDiamond(result.diamond);
                            avChatAudio.countDownTimer();
                        } else {
    ...
    public void startVideoService() {
        aVChatListener.startVideoService();
    }
