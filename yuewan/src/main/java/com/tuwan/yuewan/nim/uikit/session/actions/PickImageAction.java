package com.tuwan.yuewan.nim.uikit.session.actions;
...
import com.tuwan.yuewan.nim.uikit.common.media.picker.activity.PickImageActivity;
import com.tuwan.yuewan.nim.uikit.common.media.picker.activity.PreviewImageFromCameraActivity;
import com.tuwan.yuewan.nim.uikit.common.util.file.AttachmentStore;
...
//        showSelector(getTitleId(), requestCode, multiSelect, tempFile());
    ...
    public void showCamera() {
        show(getTitleId(), AppPhotoUtil.REQUEST_CAMERA, multiSelect, tempFile(), false);
    }

    public void showPic() {
        show(getTitleId(), AppPhotoUtil.REQUEST_PIC, true, tempFile(), true);
    }

    private void show(int titleId, final int requestCode, final boolean multiSelect, final String outPath, boolean isPic) {
        PickImageHelper.PickImageOption option = new PickImageHelper.PickImageOption();
        option.titleResId = titleId;
        option.multiSelect = multiSelect;
        option.multiSelectMaxCount = PICK_IMAGE_COUNT;
        ...
        if (isPic) {
            PickImageHelper.doPic(getActivity(), requestCode, option);
        } else {
            PickImageHelper.doCamera(getActivity(), requestCode, option);
        }
    }
    /**
     * 新增
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            case AppPhotoUtil.REQUEST_CAMERA:
                onPickImageActivityResult(requestCode, data);
                break;
            case AppPhotoUtil.PREVIEW_IMAGE_FROM_CAMERA:
    ...
    private void onPickImageActivityResult(int requestCode, Intent data) {
        ...
        getActivity().startActivityForResult(intent, AppPhotoUtil.PREVIEW_IMAGE_FROM_CAMERA);
    }
     ...
     * 新增
     */
    public void sendImage(boolean isOrig, SendImageHelper.Callback callback) {
        SendImageHelper.sendImage(getActivity(), callback, isOrig);
    }
    ...
    private void onPreviewImageActivityResult(int requestCode, Intent data) {
        if (data.getBooleanExtra(PreviewImageFromCameraActivity.RESULT_SEND, false)) {
            ...
            if (requestCode == AppPhotoUtil.PREVIEW_IMAGE_FROM_CAMERA) {
//                 TODO: 2017/11/3
                PickImageActivity.start(getActivity(), makeRequestCode(AppPhotoUtil.REQUEST_CAMERA), PickImageActivity.FROM_CAMERA, path);
            }
        }
    }
