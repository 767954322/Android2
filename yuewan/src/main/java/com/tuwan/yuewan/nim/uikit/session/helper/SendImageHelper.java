...
public class SendImageHelper {
	...
	/*****************
	 * 添加，发送AppPhotoUtil中选中的图片
     */
	public static void sendImage(Context context, final Callback callback,boolean isOrig){
		List<PhotoInfo> photos = AppPhotoUtil.getInstance().getSelectedList();
		if(photos == null) {
			Toast.makeText(context, R.string.picker_image_error, Toast.LENGTH_LONG).show();
			return;
		}
