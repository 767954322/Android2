...
/**
 * Created by zhangjie on 2017/10/26.
 */
public class AppOkHttpUrlLoader implements ModelLoader<GlideUrl, InputStream> {

    private final Call.Factory client;

    public AppOkHttpUrlLoader(Call.Factory client) {
        this.client = client;
    }
        ...
        private static Call.Factory getInternalClient() {
            if (internalClient == null) {
                synchronized (com.bumptech.glide.integration.okhttp3.OkHttpUrlLoader.Factory.class) {
                    if (internalClient == null) {
                        internalClient = OkHttpProvider.getNoCacheOkHttpClient();
                    }
                }
            }
