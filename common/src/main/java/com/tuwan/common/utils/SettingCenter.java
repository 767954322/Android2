...
public class SettingCenter {
    ...
    public static boolean getOnlyWifiLoadImage() {
        return PrefUtils.getBoolean(LibraryApplication.getInstance(), "getOnlyWifiLoadImage", false);
    }

    public static void setOnlyWifiLoadImage(boolean isEnable) {
        PrefUtils.putBoolean(LibraryApplication.getInstance(), "getOnlyWifiLoadImage", isEnable);
    }
    ...
    public static void countDirSizeTask(final CountDirSizeListener listener) {
        new Thread() {
            public void run() {
                final long result = getDirSize(LibraryApplication.getInstance().getCacheDir());
    ...
    public static void clearAppCache(final ClearCacheListener listener) {
        new Thread() {
            public void run() {
                clearFile(LibraryApplication.getInstance().getCacheDir());
                Handler handler = new Handler(Looper.getMainLooper());
                handler.post(new Runnable() {
                    @Override
                    public void run() {
